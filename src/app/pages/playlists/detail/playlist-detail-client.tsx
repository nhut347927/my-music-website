...
export const PlaylistDetailPage: React.FC = () => {
  ...
  const playlistId = idParam ? parseInt(idParam) : 1;
  ...
  const getEmbedUrl = (url: string) => {
    ...
    try {
      ...
      // Lấy video ID từ các dạng khác nhau
      if (u.hostname.includes("youtu.be")) {
        videoId = u.pathname.slice(1); // phần sau dấu "/"
      ...
      // Trả về link nhúng hợp lệ
      return videoId ? `https://www.youtube.com/embed/${videoId}${query}` : url;
    } catch {
      return url; // Trường hợp URL không hợp lệ
    }
  };
